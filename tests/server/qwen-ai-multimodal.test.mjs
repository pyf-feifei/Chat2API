...
test('Qwen AI document upload waits for parse completion with a bounded timeout', () => {
  ...
  assert.match(source, /QWEN_AI_FILE_PARSE_TIMEOUT_MS',\s*180000/)
  assert.match(source, /QWEN_AI_FILE_PARSE_KICK_INTERVAL_MS',\s*45000/)
  assert.match(source, /kicking parse fileId=/)
...
test('Qwen AI adapter request timeout is configurable for long-context document runs', () => {
  ...
  assert.match(source, /positiveNumberFromEnv\('QWEN_AI_REQUEST_TIMEOUT_MS',\s*1500000\)/)
...
test('Docker Compose exposes Qwen timeout overrides under their runtime names', () => {
  ...
  assert.match(source, /CHAT2API_QWEN_AI_STREAM_RESUME_ATTEMPTS:\s*\$\{CHAT2API_QWEN_AI_STREAM_RESUME_ATTEMPTS:-2\}/)
  assert.match(source, /CHAT2API_QWEN_AI_WORKFLOW_CONTINUATION_ATTEMPTS:\s*\$\{CHAT2API_QWEN_AI_WORKFLOW_CONTINUATION_ATTEMPTS:-2\}/)
  assert.match(source, /CHAT2API_QWEN_AI_SEMANTIC_FRESH_CHAT_ESCALATIONS:\s*\$\{CHAT2API_QWEN_AI_SEMANTIC_FRESH_CHAT_ESCALATIONS:-1\}/)
  assert.match(source, /CHAT2API_QWEN_AI_WRAPPER_LEAK_RECOVERY_ATTEMPTS:\s*\$\{CHAT2API_QWEN_AI_WRAPPER_LEAK_RECOVERY_ATTEMPTS:-2\}/)
  assert.match(source, /CHAT2API_QWEN_AI_CHAT_IN_PROGRESS_RETRY_MODE:\s*\$\{CHAT2API_QWEN_AI_CHAT_IN_PROGRESS_RETRY_MODE:-attempts\}/)
  assert.match(source, /CHAT2API_QWEN_AI_CHAT_IN_PROGRESS_RETRY_ATTEMPTS:\s*\$\{CHAT2API_QWEN_AI_CHAT_IN_PROGRESS_RETRY_ATTEMPTS:-1\}/)
  assert.match(source, /CHAT2API_QWEN_AI_RESPONSES_CONTINUATION_RETRY_ATTEMPTS:\s*\$\{CHAT2API_QWEN_AI_RESPONSES_CONTINUATION_RETRY_ATTEMPTS:-1\}/)
  assert.match(source, /CHAT2API_QWEN_AI_CONTENT_FAILOVER_ROTATION_MAX:\s*\$\{CHAT2API_QWEN_AI_CONTENT_FAILOVER_ROTATION_MAX:-0\}/)
  assert.match(source, /CHAT2API_QWEN_AI_BUSY_STORM_ACCOUNT_THRESHOLD:\s*\$\{CHAT2API_QWEN_AI_BUSY_STORM_ACCOUNT_THRESHOLD:-2\}/)
  assert.match(source, /CHAT2API_QWEN_AI_BUSY_STORM_COOLDOWN_MS:\s*\$\{CHAT2API_QWEN_AI_BUSY_STORM_COOLDOWN_MS:-600000\}/)
  assert.match(dockerfile, /ENV CHAT2API_QWEN_AI_CHAT_IN_PROGRESS_RETRY_MODE=attempts/)
  assert.match(dockerfile, /ENV CHAT2API_QWEN_AI_CHAT_IN_PROGRESS_RETRY_ATTEMPTS=1/)
  ...
  assert.match(source, /QWEN_AI_REQUEST_TIMEOUT_MS:\s*\$\{QWEN_AI_REQUEST_TIMEOUT_MS:-1500000\}/)
  ...
  assert.match(source, /QWEN_AI_FILE_PARSE_TIMEOUT_MS:\s*\$\{QWEN_AI_FILE_PARSE_TIMEOUT_MS:-180000\}/)
  ...
  assert.match(dockerfile, /ENV CHAT2API_QWEN_AI_STREAM_RESUME_ATTEMPTS=2/)
  assert.match(dockerfile, /ENV CHAT2API_QWEN_AI_SEMANTIC_FRESH_CHAT_ESCALATIONS=1/)
  assert.match(dockerfile, /ENV CHAT2API_QWEN_AI_WRAPPER_LEAK_RECOVERY_ATTEMPTS=2/)
  assert.match(dockerfile, /ENV CHAT2API_QWEN_AI_WORKFLOW_CONTINUATION_ATTEMPTS=2/)
  assert.match(dockerfile, /ENV CHAT2API_QWEN_AI_RESPONSES_CONTINUATION_RETRY_ATTEMPTS=1/)
  assert.match(dockerfile, /ENV CHAT2API_QWEN_AI_CONTENT_FAILOVER_ROTATION_MAX=0/)
  assert.match(dockerfile, /ENV CHAT2API_QWEN_AI_BUSY_STORM_ACCOUNT_THRESHOLD=2/)
  assert.match(dockerfile, /ENV CHAT2API_QWEN_AI_BUSY_STORM_COOLDOWN_MS=600000/)
  assert.match(dockerfile, /ENV QWEN_AI_REQUEST_TIMEOUT_MS=1500000/)
